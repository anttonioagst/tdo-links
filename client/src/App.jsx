  ...
  return (
    <div className="space-y-4 md:space-y-5">
      ...
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricTile icon={CheckCircle2} label="Aprovacao AI" value={`${data.approvalRate}%`} tone="brand" detail={`${data.validatedTotal} ofertas passaram`} />
...
function CommandHero({ state, data, loading, action, api }) {
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-zinc-800 bg-[linear-gradient(135deg,#050505_0%,#0a0a0a_55%,#181818_100%)] p-4 shadow-2xl shadow-black/30 md:p-5">
      <div className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
        <div className="min-w-0">
          <div className="mb-4 flex flex-wrap items-center gap-2">
            <StatusBadge tone={statusTone}>{statusText}</StatusBadge>
            <span className="rounded-md border border-zinc-800 bg-black/40 px-2.5 py-1 text-xs text-zinc-400">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            ...
            <CommandStat icon={Brain} label="Filtro premium" value={`${data.approvalRate}% passa`} tone="brand" />
          </div>
        </div>
        <div className="grid content-between gap-3 rounded-lg border border-zinc-800 bg-black/30 p-4">
          <div>
            <p className="text-xs font-medium uppercase text-zinc-500">Acao rapida</p>
            <p className="mt-2 text-sm text-zinc-300">Dispara uma busca manual sem burlar validacao, imagem ou regras de postagem.</p>
          </div>
          <ActionButton
...
function PeriodSelect({ period, setPeriod }) {
  return (
    <select
      ...
      className="h-9 rounded-md border border-zinc-700 bg-black px-3 text-sm text-zinc-200 outline-none transition focus:border-zinc-400 focus:ring-2 focus:ring-zinc-500/20"
    >
    ...
    <div className="h-[310px]">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ left: 8, right: 12, top: 12, bottom: 0 }}>
          ...
          <YAxis dataKey="label" type="category" width={86} tick={{ fill: "#a1a1aa", fontSize: 12 }} axisLine={false} tickLine={false} />
          <Tooltip content={<ChartTooltip />} />
  ...
  return (
    <div className="grid gap-3 md:grid-cols-[1fr_0.9fr] xl:grid-cols-1 2xl:grid-cols-[1fr_0.9fr]">
      ...
      <div className="space-y-2 self-center">
        {data.map((entry) => (
          <div key={entry.label} className="flex items-center justify-between rounded-md border border-zinc-800 bg-black/30 px-3 py-2 text-sm">
            <span className="flex items-center gap-2 text-zinc-300"><span className="size-2 rounded-full" style={{ background: entry.color }} />{entry.label}</span>
            <span className="font-medium text-zinc-100">{entry.value}</span>
          </div>
        ))}
      </div>
    </div>
...
function DecisionSummary({ data, state }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <StatusLine label="Produtos no inventario" value={`${(state.offers || []).length} monitorados`} tone="brand" />
          ...
          {rows.map((row) => (
            <tr key={row.id} className="border-b border-zinc-900/80">
              <td className="max-w-[340px] py-3 pr-4">
                ...
                <p className="text-xs text-zinc-600">{row.store || "Amazon"} · {row.category}</p>
              </td>
              ...
              <td className="max-w-[260px] py-3 pr-4 text-xs text-zinc-500">{row.reason}</td>
              <td className="py-3 text-xs text-zinc-500">{row.at ? formatDate(row.at) : ""}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function ChartTooltip({ active, payload, label }) {
  ...
  return (
    <div className="rounded-md border border-zinc-700 bg-black/95 px-3 py-2 shadow-xl">
      {label ? <p className="mb-1 text-xs text-zinc-500">{label}</p> : null}
      {payload.map((item) => (
        <p key={item.name || item.dataKey} className="text-xs text-zinc-200">
  ...
  return (
    <div className="space-y-5">
      <section className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <MetricTile icon={CheckCircle2} label="Validados" value={stats.validated} tone="brand" detail="passaram na AI" />
      ...
      <Panel title="Agentes do pipeline" count="Fluxo: Discovery → Validation → Creative → Publisher">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {AGENTS.map((agent) => {
            ...
            return (
              <div key={agent.key} className="rounded-lg border border-zinc-800 bg-black/30 p-4">
                ...
                <p className="text-xs leading-5 text-zinc-500">{agent.description}</p>
                <p className="mt-2 text-xs text-zinc-600">{agent.schedule}</p>
              </div>
            );
          })}
        </div>
            ...
            {publishLog.length === 0 ? (
              ...
              <div className="space-y-2">
                {publishLog.slice(0, 15).map((entry, i) => {
                  ...
                  return (
                    <div key={entry.id || i} className="flex items-center gap-3 rounded-lg border border-zinc-800 bg-black/30 px-3 py-2">
                      <div className={`size-2 rounded-full ${entry.result?.ok ? "bg-emerald-500" : "bg-rose-500"}`} />
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm text-zinc-200">{offer?.title || entry.offerId || "Deal"}</p>
                        <p className="text-xs text-zinc-500">{channelLabel(entry.channel)} • {entry.result?.providerMessageId ? `msg #${entry.result.providerMessageId}` : ""} • {entry.createdAt ? formatDate(entry.createdAt) : ""}</p>
                      </div>
        ...
        <div className="col-span-12 xl:col-span-5">
          <Panel title="Resumo do funil" count="Conversao por etapa">
            <div className="space-y-3">
              ...
              <FunnelStep label="Validados (AI)" value={stats.validated} max={stats.discovered} tone="brand" />
...
function ImageGenPanel({ offers, action, api, loading }) {
  ...
  if (offers.length === 0) {
    return <p className="text-sm text-zinc-500">Nenhuma oferta no inventario ainda.</p>;
  }
  ...
  return (
    <div className="space-y-2">
      {active.length > 0 && (
        <>
          <p className="mb-3 text-xs font-medium uppercase tracking-wide text-zinc-500">Em processamento</p>
          {active.map(offer => {
            ...
            return (
              <div key={offer.id} className="rounded-lg border border-zinc-700 bg-zinc-900/30 p-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-zinc-100">{offer.title}</p>
                    ...
                    <p className="mt-1 text-xs text-zinc-500">{pct}% concluido — estimativa: ~30s total</p>
                  </div>
                </div>
              </div>
            );
          })}
          {needsManual.length > 0 && <div className="h-px bg-zinc-800 my-3" />}
        </>
      )}
      ...
      {needsManual.length > 0 && (
        <>
          ...
          {needsManual.map(offer => (
            <div key={offer.id} className="flex items-center gap-3 rounded-lg border border-zinc-800 bg-black/30 p-3">
              {offer.imageUrl ? (
                <img src={offer.imageUrl} alt="" className="size-10 shrink-0 rounded-lg object-cover ring-1 ring-zinc-700" />
              ) : (
                <div className="grid size-10 shrink-0 place-items-center rounded-lg bg-zinc-800 text-zinc-600">
                  <ImageIcon className="size-4" />
                </div>
              )}
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm text-zinc-200">{offer.title}</p>
      ...
      {active.length === 0 && needsManual.length === 0 && (
        <div className="flex items-center gap-3 rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-4">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500">{done} ofertas com imagem AI profissional</p>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div>
      <div className="mb-1 flex items-center justify-between text-xs text-zinc-500">
        <span>{label}</span>
        <span className="font-medium text-zinc-300">{value} <span className="text-zinc-600">({percent}%)</span></span>
      </div>
          ...
          <button
            ...
            className={`rounded-lg border px-3 py-1.5 text-sm transition ${
              channelFilter === ch
                ...
                : "border-zinc-700 bg-[#0a0a0a] text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200"
            }`}
          >
            {ch === "all" ? "Todos" : channelLabel(ch)}
          </button>
        ))}
        <span className="ml-auto text-sm text-zinc-500">{filtered.length} publicacoes</span>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((entry, i) => {
            const offer = offers.find(o => o.id === entry.offerId);
            return (
              <article key={entry.id || i} className="rounded-lg border border-zinc-800 bg-[#0a0a0a] p-4">
                <div className="flex items-start gap-4">
                  {offer?.imageUrl || offer?.generatedImagePath ? (
                    <img
                      src={offer.generatedImagePath ? `/api/offers/${offer.id}/image` : offer.imageUrl}
                      alt={offer.title}
                      className="size-14 shrink-0 rounded-lg object-cover ring-1 ring-zinc-700"
                    />
                  ) : (
                    <div className="grid size-14 shrink-0 place-items-center rounded-lg bg-zinc-800 text-zinc-600">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="mb-1 flex flex-wrap items-center gap-2">
                      ...
                      {entry.result?.providerMessageId && (
                        <span className="text-xs text-zinc-500">msg #{entry.result.providerMessageId}</span>
                      )}
                      <span className="ml-auto text-xs text-zinc-500">{formatDate(entry.createdAt)}</span>
                    </div>
                    <p className="text-sm font-medium text-zinc-100">{offer?.title || entry.offerId}</p>
                    {offer?.currentPrice && (
                  ...
                  {offer?.originalUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="grid size-9 shrink-0 place-items-center rounded-lg border border-zinc-700 bg-zinc-800 text-zinc-400 hover:text-zinc-200 transition"
                    >
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-zinc-500">{rejected.length} deals descartados</p>
      </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="mb-1 flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-xs text-zinc-500">{entry.at ? formatDate(entry.at) : ""}</span>
                    </div>
...
function ConfigView({ state, loading, action, api }) {
  const telegram = state.diagnostics?.telegram;
  const configInputClass = "h-11 w-full rounded-lg border border-zinc-700 bg-black px-3 text-sm text-zinc-200 outline-none transition focus:border-zinc-400 focus:ring-2 focus:ring-zinc-500/20";
        ...
        <div className="col-span-12 xl:col-span-5 space-y-5">
          ...
          <Panel title="Telegram" count={telegram?.ready ? "Configurado" : "Revisar credenciais"}>
            <div className="space-y-2 text-sm text-zinc-400">
              <div className="flex justify-between rounded-lg border border-zinc-800 bg-black/30 px-3 py-2">
                <span>Bot token</span>
        ...
        <div className="col-span-12 xl:col-span-7 space-y-5">
          <Panel title="Busca automatica" count="Cron a cada 1h — Amazon + RSS Pelando/Zoom">
            ...
            <FormField label="URLs Amazon adicionais" help="Uma URL por linha para buscas personalizadas.">
              <textarea
                className="min-h-28 w-full rounded-lg border border-zinc-700 bg-black p-3 text-sm text-zinc-200 outline-none transition focus:border-zinc-400 focus:ring-2 focus:ring-zinc-500/20"
  ...
  return (
    <div className="space-y-2">
      {recent.map((entry, i) => {
        const offer = offers.find(o => o.id === entry.offerId);
        return (
          <div key={entry.id || i} className="flex items-center gap-3 rounded-lg border border-zinc-800 bg-black/30 px-3 py-2.5">
            <Activity className="size-3.5 shrink-0 text-emerald-400" />
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm text-zinc-200">{offer?.title || entry.offerId}</p>
              <p className="text-xs text-zinc-500">{channelLabel(entry.channel)} • {formatDate(entry.createdAt)}</p>
            </div>
            <StatusBadge tone="success">{channelLabel(entry.channel)}</StatusBadge>
          </div>
        );
...
function StatusLine({ label, value, tone = "neutral" }) {
  ...
  }[tone] || "border-zinc-700 text-zinc-200";
    ...
    <div className="overflow-x-auto">
      <div className="grid min-w-[720px] grid-cols-[44px_repeat(24,minmax(0,1fr))] gap-1">
        ...
        {Array.from({ length: 24 }, (_, hour) => (
          <div key={hour} className="text-center text-[10px] text-zinc-600">{hour}</div>
        ))}
        {heatmap.map((row, day) => (
          <React.Fragment key={day}>
            <div className="text-xs text-zinc-600">{dayLabels[day]}</div>
            {row.map((value, hour) => (
              <div
                key={hour}
...
function Bars({ title, items }) {
  return (
    <div>
      <h4 className="mb-4 text-sm font-medium text-zinc-300">{title}</h4>
      <div className="space-y-4">
        {items.map((item) => (
          <div key={item.label}>
            <div className="mb-1 flex items-center justify-between text-xs text-zinc-500">
...
function buildChannelChart(published) {
  ...
  return Object.entries(countBy(published, (entry) => channelLabel(entry.channel)))
    .sort((a, b) => b[1] - a[1])
    .map(([label, value]) => ({ label, value, color: colors[label] || "#a1a1aa" }));
}
...
function heatClass(level) {
  ...
  ][level] || "border-zinc-800 bg-zinc-800";
}
